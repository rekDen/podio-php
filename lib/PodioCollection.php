...
class PodioCollection implements IteratorAggregate, ArrayAccess, Countable
{
    ...
    /**
     * Convert collection to json encoded string
     * @return string
     */
    public function __toString()
    {
        return $this->as_json(true);
    }


    /**
     * Convert collection to array or json
     *
     * @param bool $encode
     * @return array|string
     */
    public function as_json($encode = false)
    {
        $items = array();
        foreach ($this->__items as $item) {
            $items[$item->id] = $item->as_json(false);
        }
        return $encode ? json_encode($items) : $items;
    }
